...
| `SITE_ROUTER_HOST`                                                                                                   | unset                                                                              | Optional dedicated host for HTTP actions, e.g. `defysite.com`. Site URLs become `<name>.<host>` instead of `<name>-site.<ROUTER_HOST>`. Legacy hostname keeps working. |
...
`SITE_ROUTER_HOST` (e.g. `defysite.com`) and they move to
`<name>.${SITE_ROUTER_HOST}` — so `happy-otter-123.defyhost.com` for the
database and `happy-otter-123.defysite.com` for actions, instead of the `-site`
suffix.
